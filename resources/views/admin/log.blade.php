<x-app-layout>
                    ...
                    @foreach ($pivotCheckouts->groupBy('checkout_id') as $index => $items)
                        ...
                        <div class="card mb-4 p-6 bg-white shadow-lg rounded-lg border border-gray-200">
                            ...
                            <div class="relative mt-4">
                                <select class="order-track-dropdown bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                    <option value="Pending" {{ $items->first()->order_track == 'pending' ? 'selected' : '' }}>Pending</option>
                                    <option value="Packing" {{ $items->first()->order_track == 'packing' ? 'selected' : '' }}>Packing</option>
                                    <option value="Shipping" {{ $items->first()->order_track == 'shipping' ? 'selected' : '' }}>Shipping</option>
                                    <option value="Delivered" {{ $items->first()->order_track == 'delivered' ? 'selected' : '' }}>Delivered</option>
                                </select>
                            ...
                            <div class="relative mt-1">
                                <select class="status-dropdown bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                    <option value="Unsettled" {{ $items->first()->payment_status == 'unsettled' ? 'selected' : '' }}>Belum Lunas</option>
                                    <option value="Settled" {{ $items->first()->payment_status == 'settled' ? 'selected' : '' }}>Lunas</option>
                                </select>
                                <input type="hidden" class="checkout-id" value="{{ $checkoutId }}">                                    
                            </div>
                        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.order-track-dropdown').focus(function() {
                oldTrack = $(this).val();  // Store the current status
            }).change(function() {
                let newTrack = $(this).val().toLowerCase();  // Convert the new status to lowercase
                let capitalizedNewOrder = newTrack.charAt(0).toUpperCase() + newTrack.slice(1);  // Capitalize the first letter
                let checkoutId = $(this).siblings('.checkout-id').val();
    
                // Check if the new status is different from the old status
                if (newTrack !== oldTrack.toLowerCase()) {
                    let confirmMessage = `Apakah anda yakin ingin merubah proses order "${oldTrack}" menjadi "${capitalizedNewOrder}" pada id transaksi ${checkoutId}?`;
    
                    if (confirm(confirmMessage)) {
                        $.ajax({
                            url: '/update-order/' + checkoutId,  // Sertakan checkoutId dalam URL
                            type: 'POST',
                            data: {
        ...
        $(document).ready(function() {
            ...
            $('.status-dropdown').focus(function() {
                ...
                if (newStatus !== oldStatus.toLowerCase()) {
                    let confirmMessage = `Apakah anda yakin ingin merubah status pembayaran "${oldStatus}" menjadi "${capitalizedNewStatus}" pada id transaksi ${checkoutId}?`;
